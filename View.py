...
class view:
    ...
    def setup_player(self):
        ...
        self.playList.configure(font="Gabriola 12 bold")
        ...
        self.previousButton.bind("<Button>",self.play_previous_song)
        self.Button9.config(command=self.add_song_to_favourite)
        self.Button10.config(command=self.remove_song_from_favourites)
        self.Button11.config(command=self.load_songs_from_favourites)

    def close_window(self):
        ans = messagebox.askyesno("App closing!","DO you want to quit ?")
        if ans is True:
            messagebox.showinfo("Thank you!","thanks for using the app MOUZIKKA")
    ...
    def change_volume(self,val):
        volume_level = float(val)/100 # as the method "set_volume" of mixer takes values b/w 0.1 to 0.9
    ...
    def add_song(self):
        ...
        self.playList.insert(tk.END,song_name) # for adding song to playlist
        # code for  randomly changing the color of selected songs :-
    ...
    def play_song(self):
        ...
        self.set_song_index_tuple = self.playList.curselection() # it will assign the list of index of selected song to "set_song_index_tuple" as tuple
        #print(type(self.set_song_index_tuple))
        #print(self.set_song_index_tuple)
        try:
            if len(self.set_song_index_tuple)==0: # i.e. if song is not selected and play butten is pressed
                raise NoSongSelectedError("Please select a song to play.")

            self.song_name = self.playList.get(self.set_song_index_tuple[0]) # it will assign the song at 1st index from the tuple "set_song_index_tuple" to "song_name"
    ...
    def show_timer(self,total_sec):
        self.curr_sec=0
        self.prev_prec_incr=0
        self.songProgress.stop()
        while self.curr_sec<= total_sec  and mixer.music.get_busy():
            ...
            percent_increment= self.curr_sec *100/total_sec
            if (int(percent_increment)-int(self.prev_prec_incr)>=1):
                self.songProgress.step()
                self.prev_prec_incr = percent_increment
            time.sleep(1)

        if(abs(self.curr_sec-total_sec)==0 or abs(self.curr_sec-total_sec)==1):
            self.load_next_song()


    def load_next_song(self):

        n=int(self.set_song_index_tuple[0])
        if n==(self.playList.getsize()-1):
            n=0
        else:
            n=n+1
        self.playList.selection_set(n)
        self.play_song()
        return

    def play_previous_song(self,e):
        self.set_index_tuple = self.playList.curselection()
        try:
            if len(self.set_index_tuple)==0: # i.e. if song is not selected and play butten is pressed
                raise NoSongSelectedError("Please select a song first!.")

            n= self.playList.getsize()
            if n==1:# if there is only 1 song in playlist
                self.play_song()

            else : # if the there are more than 1 songs in playlist
                if self.set_index_tuple[0]==0 :# if the selected song is 1st song
                    self.playList.selection_set(n-1)
                    self.play_song()

            '''self.previous_song_index=int(self.sel_index_tuple[0])-1
            self.my_player.play_song()'''
    ...
    def remove_song(self):
        ...
        try:
            if len(self.sel_index_tuple) == 0 :
                raise NoSongSelectedError("Please select any song to remove.")

            song_name=self.playList.get(self.sel_index_tuple[0])

            self.playList.delete(self.sel_index_tuple[0]) # removing song from playlist
    ...
    def add_song_to_favourite(self):
        self.set_index_tuple = self.playList.curselection()
        try:
            if len(self.set_index_tuple) == 0:  # i.e. if song is not selected and favourite button is pressed
                raise NoSongSelectedError("Please select a song first!.")
            n = self.set_index_tuple[0]
            song_name = self.playList.get(n)
    ...
    def load_songs_from_favourites(self):
        try:
            ...
            result_tuple = self.my_player.load_songs_from_favourites()
            result = result_tuple[0]
            if "No song present" in result :
                messagebox.showinfo("info...","No song present in your favourites ")
                return
            else:
                song_dict = result_tuple[1]
                self.playList.delete(0,tk.END)
                for song_name in song_dict:
                    self.playList.insert(tk.END,song_name)
                    print("Song added from Database :", song_name)
                rcolor = lambda: random.randint(0, 255)
                ...
                my_color = "#" + red + green + blue
                self.playList.config(fg=my_color)
                messagebox.showinfo("info...","List populated from favourites ! ")
        ...
        try:
            ...
            for string in result:
                if string=="deleted":
                    ...
                    print("Song removed from favourites! -" + song_name)

            # code for checking that selected song is playing, if yes then stop it :
